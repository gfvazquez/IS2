...
psql -c "update userstory_userstory set estado = 'Incompleto' where userstory_userstory.sprint_id = 
(select proyecto_flujoproyecto.sprint_id 
	from proyecto_flujoproyecto 
	where estado='HalfDone') and userstory_userstory.estado not ilike 'Validado'" -d is2 -U postgres
